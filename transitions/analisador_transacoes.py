...
def analisar_transacao(lista_transacoes):
    ...
    salva(".venv/transition/transacoes.json", conteudo)
    return json_resultado

def gerar_parecer(transacao):
    print("2.Gerando um parecer para a transição:", transacao["id"])
    prompt_sistema = f"""
    ...
    salva(".venv/transition/parecer.json", conteudo)
    return conteudo

lista_de_transacoes = carrega(".venv/transition/transacoes.csv")
